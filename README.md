...
`./run_adaptive.ps1` 输出到 `output/adaptive_v10/`，其输入 CSV 与质量满分第五版
逐字节相同。新增相对误差 Ridge/稳健拟合，以及仅使用已经到达标签的周期更新。
本地两个目标均小幅改善，官方分数尚未知。**此分支的部分参数在部署期滚动更新**，
并非整段冻结。协议、验证、局限和复现见 `ADAPTIVE_V10_REPORT.md`。
